...
class Game:
    def __init__(self, allowed_misses: int = 6):
        if allowed_misses < 5 or allowed_misses > 8:
            raise ValueError("Number of mistake should be between 5 and 8")
    ...
    def generate_word(self) -> str:
        filename = r"data/WordsStockRus.txt"
    ...
    def guess_letter(self, letter: str) -> Iterable[str]:
        if self.__tries_counter == self.__allowed_misses:
            raise InvalidOperationError(f"Exceeded the misses number. Allowed {self.__allowed_misses}")
